...
This lab covers "differential abundance" using metagenomics data, emphasizing using the outputs of anvi'o's `anvi-estimate-scg-taxonomy` function. 
(You could also use the output of `anvi-profile-blitz`.)

It specifically covers the differential abundance method `radEmu`. We like these methods because radEmu estimates changes in the "absolute abundance" of microbial taxa, even without absolute quantification data. Here, "absolute abundance" could be interpreted on the cell count, cell concentration or DNA concentration scale. Yes! It’s true! (We talked about this in lecture.)

We'll illustrate its use on the the Healthy Dairy Workers dataset. We're interested in estimating fold-changes in microbial abundances before compared to after commencing dairy work. 
...
```{r}
meta <- read.table("data/metadata.tsv", header = TRUE, row.names = 1) %>% 
  as_tibble %>% 
...
```{r, message=FALSE}
coverages <- list.files("data/11_MGX_TAXONOMY/",pattern = "^S.*txt", full.names = T) %>% 
  lapply(FUN = read_tsv) %>% 
  bind_rows() %>%
  ...
  dplyr::select(-t_domain, -t_phylum, -t_class, -t_order, -t_family, -t_genus, -t_species) %>%
  pivot_wider(values_from = coverage, values_fill = 0, names_from = taxa) %>%
...
```{r}
coverages <- coverages %>%
  filter(!endsWith(rownames(coverages),"B"))
...
```{r}
coverages <- coverages %>% dplyr::select(where(~ sum(.x) != 0))
dim(coverages)
```
...
The printed output is sorted by the largest estimated effect sizes (fold differences), but how do we look at a specific species? The way to access estimated coefficients and confidence intervals from the model is with `hdw_fit$coef`. Let's look at one, then save the estimates in a vector. 
...
meta %>% 
  ...
  dplyr::select(sample_id, exposed, coverage)
```
...
We estimate that the average abundance of `r hdw_fit$coef[taxa_to_test[1], "category"] %>% sub(".*s__", "", .)` in metagenomes is `r 1/(hdw_fit$coef[taxa_to_test[1], "estimate"] %>% exp %>% signif(2))` times greater *before* commencing dairy work, when compared to the typical fold-differences in the average abundance of taxa across these visits (95% CI `r 1/(hdw_fit$coef[taxa_to_test[1], "upper"] %>% exp %>% signif(2))`--`r 1/(hdw_fit$coef[taxa_to_test[1], "lower"] %>% exp %>% signif(2))`). 

In many data analyses, we'd like to run reliable tests for all taxa that we have measured. We could run robust score tests for every taxon in this analysis, but it will take a long time to run all tests serially. They can easily be run in parallel on a computing cluster (for example, if you are in a VM!). 

We show you how this looks in the following chunk. DO NOT RUN UNLESS YOU HAVE THE TIME!
...
if (.Platform$OS.type != "windows" & !identical(Sys.getenv("GITHUB_ACTIONS"), "true")) {
  ...
  score_res <- parallel::mclapply(1:5,
                      emuTest,
                      mc.cores = ncores)
} else {
